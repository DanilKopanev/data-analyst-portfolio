...
--
-- Рекомендации:
-- Создать специальные программы и награды для наиболее активных доноров: донорские значки (уже есть), сертификаты, публичное признание и дополнительные бонусы.
...
--
-- Для корректного анализа, если у пользователя может быть несколько соцсетей, нужно «развернуть» данные так, чтобы каждый пользователь учитывался по каждому действительно привязанному каналу
SELECT 'ВКонтакте' AS социальная_сеть,
		COUNT(DISTINCT id) AS количество_доноров,
		AVG(confirmed_donations) AS среднее_количество_донаций
FROM donorsearch.user_anon_data
WHERE autho_vk
UNION ALL
SELECT 'Одноклассники',
		COUNT(DISTINCT id),
...
WHERE autho_google
UNION ALL
SELECT 'Без авторизации через соцсети',
		COUNT(DISTINCT id),
		AVG(confirmed_donations)
FROM donorsearch.user_anon_data
WHERE NOT (autho_vk OR autho_ok OR autho_tg OR autho_yandex OR autho_google);
...
WITH donor_activity AS (
  SELECT 
    user_id,
    COUNT(*) AS total_donations, -- Считаем, сколько раз каждый пользователь сдавал кровь
    (MAX(donation_date) - MIN(donation_date)) AS activity_duration_days, -- Считаем разницу между первой и последней сдачей для пользователя
    CASE 
      WHEN COUNT(*) > 1 THEN (MAX(donation_date) - MIN(donation_date)) / (COUNT(*) - 1) -- Если у пользователя несколько донаций, считаем средний интервал между ними
      ELSE NULL -- Если донация одна, интервал не считаем, ставим NULL
    END AS avg_days_between_donations
  FROM donorsearch.donation_anon
  GROUP BY user_id -- Группируем по каждому пользователю
)
SELECT
  CASE 
    WHEN total_donations = 1 THEN '1 донация' -- Если у пользователя одна сдача, это отдельная группа
    ELSE '2 и более' -- Все остальные (повторные доноры)
  END AS donation_group,
  COUNT(user_id) AS donor_count, -- Считаем число доноров в каждой группе
  AVG(total_donations) AS avg_donations_per_donor, -- Среднее количество донаций на донора в группе
  AVG(activity_duration_days) AS avg_activity_duration_days, -- Средняя длительность активности в днях (от первой до последней сдачи)
  AVG(avg_days_between_donations) AS avg_days_between_donations -- Средний интервал между донациями (NULL для однократных)
FROM donor_activity
GROUP BY donation_group -- Итоговая группировка по типу донора: однократный или повторный
ORDER BY donation_group;
-- Однократных доноров больше, но повторные приносят системе намного больше пользы — у них выше среднее число донаций и вовлечённость.
-- Чтобы повысить эффективность, важно работать над возвратом однократных доноров, мотивируя их сдать кровь повторно через коммуникации, напоминания и небольшие поощрения.
...
WITH planned_donations AS (
  ...
  -- Получаем список уникальных запланированных донаций по пользователю, дате и типу донации
),
actual_donations AS (
  SELECT DISTINCT user_id, donation_date
  FROM donorsearch.donation_anon
  -- Получаем список уникальных фактических донаций (кто реально сдал и когда)
),
planned_vs_actual AS (
  SELECT
    ...
    CASE
	    ...
	    ELSE 0
	    END AS completed
    -- completed = 1, если по этому пользователю и дате есть совпадение с реальным фактом сдачи; иначе 0
  FROM planned_donations pd
  LEFT JOIN actual_donations ad ON pd.user_id = ad.user_id AND pd.donation_date = ad.donation_date
  -- Сопоставляем каждую плановую сдачу с фактическим выполнением по user_id и дате; теперь видно, выполнено или нет
)
SELECT
  donation_type,
  COUNT(*) AS total_planned_donations, -- Число плановых донаций по типу
  SUM(completed) AS completed_donations, -- Число реально выполненных донаций по типу
  ROUND(SUM(completed) * 100.0 / COUNT(*), 2) AS completion_rate -- Доля выполнения в процентах
FROM planned_vs_actual
GROUP BY donation_type;
-- Группируем результат по типу донации: всего планировалось, сколько выполнено, и какова эффективность в процентах    
